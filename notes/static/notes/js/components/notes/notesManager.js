...
export class NotesManager {
  ...
  #appendNoteToList(id, noteTitle) {
    const noteTemplate = this.notesList.querySelector("#note-item");
    const noteClone = noteTemplate.content.cloneNode(true);

    const note = noteClone.querySelector(".note-item");
    note.dataset.id = id;

    const title = note.querySelector(".note-item span");
  ...
  clearList() {
    this.notesList.querySelectorAll(".note-item").forEach((note) => note.remove());
  }
